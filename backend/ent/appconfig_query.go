...
func (acq *AppConfigQuery) QueryProfile() *ProfileQuery {
	query := (&ProfileClient{config: acq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(appconfig.Table, appconfig.FieldID, selector),
...
func (acq *AppConfigQuery) Clone() *AppConfigQuery {
	...
	return &AppConfigQuery{
		...
		withProfile: acq.withProfile.Clone(),
...
func (acq *AppConfigQuery) WithProfile(opts ...func(*ProfileQuery)) *AppConfigQuery {
	query := (&ProfileClient{config: acq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	acq.withProfile = query
	return acq
}
...
func (acq *AppConfigQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AppConfig, error) {
	...
	if query := acq.withProfile; query != nil {
		if err := acq.loadProfile(ctx, query, nodes, nil,
			func(n *AppConfig, e *Profile) { n.Edges.Profile = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (acq *AppConfigQuery) loadProfile(ctx context.Context, query *ProfileQuery, nodes []*AppConfig, init func(*AppConfig), assign func(*AppConfig, *Profile)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*AppConfig)
	for i := range nodes {
	...
	query.Where(predicate.Profile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(appconfig.ProfileColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.app_config_profile
		if fk == nil {
			return fmt.Errorf(`foreign-key "app_config_profile" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "app_config_profile" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
