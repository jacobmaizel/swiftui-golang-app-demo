...
func (aq *ActionQuery) QuerySender() *ProfileQuery {
...
func (aq *ActionQuery) Clone() *ActionQuery {
	...
	return &ActionQuery{
		...
		withSender: aq.withSender.Clone(),
...
func (aq *ActionQuery) WithSender(opts ...func(*ProfileQuery)) *ActionQuery {
	query := (&ProfileClient{config: aq.config}).Query()
...
func (aq *ActionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Action, error) {
	...
	if query := aq.withSender; query != nil {
		if err := aq.loadSender(ctx, query, nodes, nil,
			func(n *Action, e *Profile) { n.Edges.Sender = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ActionQuery) loadSender(ctx context.Context, query *ProfileQuery, nodes []*Action, init func(*Action), assign func(*Action, *Profile)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Action)
	for i := range nodes {
		if nodes[i].action_sender == nil {
			continue
		}
		fk := *nodes[i].action_sender
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "action_sender" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
